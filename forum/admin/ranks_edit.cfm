...
<cfif isDefined("form.cancel.x") or not isDefined("url.id") or not len(url.id)>
	<cflocation url="z.ranks.cfm" addToken="false">
</cfif>
...
<cfif isDefined("form.save.x")>
	...
	<cfif not len(errors)>
		...
		<cfif url.id neq 0>
			<cfset APPLICATION.FORUM.Rank.saveRank(url.id, rank)>
		<cfelse>
			<cfset APPLICATION.FORUM.Rank.addRank(rank)>
		</cfif>
		<cfset msg = "Rank, #rank.name#, has been updated.">
		<cflocation url="z.ranks.cfm?msg=#urlEncodedFormat(msg)#" addToken="false">
	</cfif>
</cfif>

<!--- get conference if not new --->
<cfif url.id neq "0">
	<cfset rank = APPLICATION.FORUM.Rank.getRank(url.id)>
