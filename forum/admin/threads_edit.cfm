...
<cfif isDefined("form.cancel.x") or not isDefined("url.id") or not len(url.id)>
	<cflocation url="z.threads.cfm" addToken="false">
</cfif>
...
<cfif isDefined("form.save.x")>
	...
	<cfif not len(errors)>
		...
		<cfif url.id neq 0>
			<cfset APPLICATION.FORUM.Thread.saveThread(url.id, thread)>
		<cfelse>
			<cfset APPLICATION.FORUM.Thread.addThread(thread)>
		</cfif>
		<cfset msg = "Thread, #thread.name#, has been updated.">
		<cflocation url="z.threads.cfm?msg=#urlEncodedFormat(msg)#" addToken="false">
	</cfif>
</cfif>

<!--- get thread if not new --->
<cfif url.id neq 0>
	<cfset thread = APPLICATION.FORUM.Thread.getThread(url.id)>
...
<!--- get all forums --->
<cfset forums = APPLICATION.FORUM.Forums.getForums(false)>

<cfset author = APPLICATION.FORUM.User.getUserNameFromId(thread.useridfk)>
