...
<cfif isDefined("form.cancel.x") or not isDefined("url.id")>
	<cflocation url="z.groups.cfm" addToken="false">
</cfif>
...
<cfif isDefined("form.save.x")>
	...
	<cfif not len(errors)>
		...
		<cfif url.id neq 0>
			<cftry>
				<cfset APPLICATION.FORUM.User.saveGroup(form.id, form.group)>
				<cfcatch>
		...
		<cfelse>
			<cftry>
				<cfset APPLICATION.FORUM.User.addGroup(form.group)>
				<cfcatch>
					<cfset errors = cfcatch.message & " " & cfcatch.detail>
				</cfcatch>
			</cftry>
		...
		<cfif not len(errors)>
			...
			<cflocation url="z.groups.cfm?msg=#urlEncodedFormat(msg)#" addToken="false">
		</cfif>
	</cfif>
</cfif>
...
<cfif url.id neq 0>
	<cfset group = APPLICATION.FORUM.User.getGroup(url.id)>
