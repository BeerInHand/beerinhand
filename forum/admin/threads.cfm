...
<cfif isDefined("form.mark") and len(form.mark)>
	<cfloop index="id" list="#form.mark#">
		<cfset APPLICATION.FORUM.Thread.deleteThread(id)>
	</cfloop>
	<cfset url.msg = "Thread(s) deleted.">
</cfif>

<!--- get threads --->
<cfset threads = APPLICATION.FORUM.Thread.getThreads(bActiveOnly=false,start=url.start, max=10, sort="#url.sort# #url.dir#", search=form.search)>
...
<cfmodule template="../tags/datatablenew.cfm"
		  ...
		  classList="left_15,left_15,left_20,left_15,left_10 center,left_10 center,left_10 center"
		  editlink="z.threads_edit.cfm" linkcol="name" label="Thread" linkappend="&search=#urlEncodedFormat(form.search)#"/>
