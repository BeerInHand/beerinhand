...
<cfif isDefined("form.cancel.x") or not isDefined("url.id") or not len(url.id)>
	<cflocation url="z.messages.cfm" addToken="false">
</cfif>
...
<cfif isDefined("form.save.x")>
	...
	<cfif not len(errors)>
		...
		<cfif structKeyExists(form, "removefile") and isDefined("message.attachment") and fileExists(APPLICATION.DISK.ATTACH & "/" & message.filename)>
			<cffile action="delete" file="#APPLICATION.DISK.ATTACH#\#message.filename#">
		...
		<cflocation url="z.messages.cfm?msg=#urlEncodedFormat(msg)#" addToken="false">
	</cfif>
</cfif>

<cfset message = APPLICATION.FORUM.Message.getMessage(url.id)>
<cfset thread = APPLICATION.FORUM.Thread.getThread(message.threadidfk)>
<cfset threads = APPLICATION.FORUM.Thread.getThreads(forumidfk=thread.forumidfk,sort="name")>
...
<div class="row_1">
	...
	<a href="f.../attachment.cfm?id=#url.id#" class="cb_text">#message.attachment#</a>
