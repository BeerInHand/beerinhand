var sum = { cnt: 0, vol: 0, grv: 0, srm: 0, eff: 0, ibu: 0 };
...
brewLogSetSum = function() {
	$("#dataEmpty").toggle(sum.cnt==0);
	$("#dataFull").toggle(sum.cnt!=0);
...
brewlogLoad = function(node, filter) {
	...
	if (!filter) {
		...
		for (var key in filter) {
			title = dataHelper(key).cap;
			title = (title ? " : " + title : "") + " : " + filter[key];
		}
...
brewlogScatter = function(ROW, $row) {
	...
	if (ROW.RE_PRIVACY && !isUserBrewer) {
		$row.find(".iconedit").find(".ui-button-icon-primary").removeClass("ui-icon-pencil").addClass("ui-icon-locked");
		$row.find(".iconview").find(".ui-button-icon-primary").removeClass("ui-icon-script").addClass("ui-icon-locked");
	}
	brewlogSum(ROW);
}
...
brewlogInit = function(filter) {
	...
	$trBrewLog.remove();
	$("#divSubPage").find(".hide").removeClass("hide");
