<cfcomponent displayName="Blog" output="false" hint="BlogCFC by Raymond Camden">
	...
	<cffunction name="addComment" access="remote" returnType="uuid" output="false" hint="Adds a comment.">
		...
		<cfquery datasource="#instance.dsn#">
			INSERT INTO tblblogcomments(
				id,entryidfk,name,email,website,comment,posted,subscribe,moderated,killcomment,subscribeonly,usid
			) VALUES (
	...
	<cffunction name="approveComment" access="public" returnType="void" output="false" hint="Approves a comment.">
		...
		<cfquery datasource="#instance.dsn#">
			update tblblogcomments
				set moderated = <cfqueryparam value="1" cfsqltype="CF_SQL_TINYINT">
...
<!--- 			and			tblblogentries.blog = <cfqueryparam value="#instance.name#" cfsqltype="CF_SQL_VARCHAR" maxlength="50"> --->
	...
	<cffunction name="getComment" access="remote" returnType="query" output="false" hint="Gets a specific comment by comment ID.">
		...
		<cfset var getC = "">
		<cfquery name="getC" datasource="#instance.dsn#">
			select		id, entryidfk, name, email, website, comment, posted, subscribe, moderated, killcomment, usid
	...
	<cffunction name="getComments" access="remote" returnType="query" output="false" hint="Gets all comments for an entry ID.">
		...
		<cfquery name="getC" datasource="#instance.dsn#">
			SELECT tblblogcomments.id, tblblogcomments.name, tblblogcomments.email, tblblogcomments.website,
					 tblblogcomments.comment, tblblogcomments.posted, tblblogcomments.subscribe, tblblogentries.title as entrytitle, tblblogcomments.entryidfk,
					 CAST(md5(tblblogcomments.email) AS CHAR) AS gravatar, IF(LENGTH(avatar)=0, 'zombatar.jpg', avatar) AS avatar, tblblogcomments.usid
			  FROM tblblogcomments
					 INNER JOIN tblblogentries ON tblblogcomments.entryidfk = tblblogentries.id
					 LEFT OUTER JOIN forum_users AS fu ON fu.usid = tblblogcomments.usid
			 WHERE 1=1
			<cfif structKeyExists(arguments, "search")>
				AND (
						tblblogcomments.comment LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#ARGUMENTS.search#%">
						OR
	...
	<cffunction name="getEntry" access="remote" returnType="struct" output="false" hint="Returns one particular entry.">
		...
		<cfquery name="getIt" datasource="#instance.dsn#">
			select	tblblogentries.id, tblblogentries.title, date_add(posted, interval #instance.offset# hour) as posted,
						tblblogentries.body,
						tblblogentries.morebody, tblblogentries.alias, tblblogentries.allowcomments,
			...
			from		tblblogentries, bih.users
			where		tblblogentries.id = <cfqueryparam value="#ARGUMENTS.id#" cfsqltype="CF_SQL_VARCHAR" maxlength="35">
			and			tblblogentries.blog = <cfqueryparam value="#instance.name#" cfsqltype="CF_SQL_VARCHAR" maxlength="50">
			and			tblblogentries.username = bih.users.us_user
		</cfquery>
	...
	<cffunction name="getEntries" access="remote" returnType="struct" output="false" hint="Returns entries. Allows for a params structure to configure what entries are returned.">
		...
		from	tblblogentries
			...
			where		1=1
						and tblblogentries.blog = <cfqueryparam value="#instance.name#" cfsqltype="CF_SQL_VARCHAR" maxlength="50">
<!--- 	and tblblogentries.username = tblusers.username	and	tblblogentries.blog = tblusers.blog --->
			<cfif structKeyExists(ARGUMENTS.params,"lastXDays")>
		...
		<cfquery name="getEm" datasource="#instance.dsn#" maxrows="#ARGUMENTS.params.maxEntries+ARGUMENTS.params.startRow-1#">
			select
					...
					tblblogentries.alias, date_add(posted, interval #instance.offset# hour) as posted,
	...
	<cffunction name="getNumberUnmoderated" access="public" returntype="numeric" output="false" hint="Returns the number of unmodderated comments for a specific blog entry.">
		<cfset var getUnmoderated = "" />
		<cfquery name="getUnmoderated" datasource="#instance.dsn#">
	...
	<cffunction name="getRelatedBlogEntries" access="remote" returntype="query" output="true" hint="returns related entries for a specific blog entry.">
		...
		<cfquery name="getThisRelatedEntry" datasource="#instance.dsn#">
			select tblblogentries.id, tblblogentries.title, tblblogentries.posted, tblblogentries.alias, tblblogcategories.categoryname
			  from tblblogcategories
					 inner join tblblogentriescategories on tblblogcategories.categoryid = tblblogentriescategories.categoryidfk
					 inner join tblblogentries on tblblogentriescategories.entryidfk = tblblogentries.id
	...
	<cffunction name="getUnmoderatedComments" access="remote" returnType="query" output="false" hint="Gets unmoderated comments for an entry.">
		<cfargument name="id" type="uuid" required="false">
		<cfargument name="sortdir" type="string" required="false" default="asc">
		...
		<cfquery name="getC" datasource="#instance.dsn#">
			SELECT tblblogcomments.id, tblblogcomments.name, tblblogcomments.email, tblblogcomments.website, tblblogcomments.comment, tblblogcomments.posted, tblblogcomments.subscribe, tblblogentries.title AS entrytitle, tblblogcomments.entryidfk
	...
	<cffunction name="killComment" access="public" returnType="void" output="false" hint="Deletes a comment based on a separate uuid to identify the comment in email to the blog admin.">
...
<a href="#APPLICATION.PATH.FULL#/b.unsubscribe.cfm?email=#email#&amp;token=#token#">#APPLICATION.PATH.FULL#/b.unsubscribe.cfm?email=#email#&amp;token=#token#</a>
				...
				<cfif findNoCase("%unsubscribe%", ARGUMENTS.message)>
					...
					<cfelse>
						...
						<cfif mailType is "text">
							<cfset ulink = ulink & "#chr(10)#Delete this comment: #instance.blogURL#?killcomment=#comment.killcomment#">
						<cfelse>
							<cfset ulink = ulink & " <a href=""#instance.blogURL#?killcomment=#comment.killcomment#"" style=""font-size:8pt;text-decoration:underline;color:##7d8524;text-decoration:none;"">Delete</a>">
						</cfif>
	...
	<cffunction name="saveComment" access="public" returnType="uuid" output="false" hint="Saves a comment.">
		...
		<cfquery datasource="#instance.dsn#">
			...
			moderated= <cfqueryparam value="#ARGUMENTS.moderated#" cfsqltype="CF_SQL_TINYINT">
			where	id = <cfqueryparam value="#ARGUMENTS.commentid#" cfsqltype="CF_SQL_VARCHAR" maxlength="35">
		</cfquery>
	...
	<cffunction name="setModeratedComment" access="public" returnType="void" output="false" roles="" hint="Sets a comment to approved">
