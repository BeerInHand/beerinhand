				...
				<tr>
					...
					<td>
						<cfif allCats.RecordCount>
							...
							<cfloop query="allCats"><option value="#categoryID#" <cfif isDefined("form.categories") and listFind(form.categories,categoryID)>selected</cfif>>#categoryName#</option></cfloop>
							</select>
						</cfif>
					</td>
				</tr>
...
<!--- 				<tr>
					...
					<td><input type="text" name="manualenclosure" id="manualenclosure" class="textInput"> <span class="moderated">...Must know existing file name</span></td>
				</tr>
			<cfif len(form.oldenclosure)>
				<tr>
				...
				<tbody class="bih-grid-form nobevel">
					...
					<tr>
						...
						<td>
							<select id="categories_dropdown" size="4" style="width:50%">
								<cfloop query="allCats"><option value="#categoryid#">#categoryname#</option></cfloop>
							</select>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							<span class="moderated">(clicking removes an entry)</span>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="previewEntry">
			...
			<div style="margin-bottom: 10px">#APPLICATION.BLOG.resourceBundle.getResource("postedat")# : #APPLICATION.BLOG.localeUtils.dateLocaleFormat(posted)# #APPLICATION.BLOG.localeUtils.timeLocaleFormat(posted)#
				...
				<cfloop index="x" from=1 to="#listLen(form.categories)#">
					<a href="#SESSION.BROG.makeCategoryLink(listGetAt(form.categories,x))#">#SESSION.BROG.getCategory(listGetAt(form.categories,x)).categoryname#</a><cfif x is not listLen(form.categories)>,</cfif>
				</cfloop>
			</div>
